...
Für Softwareentwickler wie auch für andere Berufe gilt: Ein Verbessern und Erweitern der Gegebenheiten wird grundsätzlich aus der Nutzlichkeit des Gegebenen begründet.
Die daraus folgende Kritik ist immer im Sinne der Sache und nie bezogen auf die Fähigkeiten des Entwicklers.
...
Folgenden wird die Klasse SerialDate aus dem Packet org.jfree.date restruktoriert.
Refactoring wird in zwei Schritten durchgeführt, zuerst werden Fehler, Bugs und ungenuzter Code durch Tests gefunden und behoben.
Anschließend ist der Code zwar funktionell und Fehlerfrei jedoch noch nicht erweitert und noch lange nicht vereinbar mit den best practices für guten code.
...
Was ist wichtig für Tests?
Grundsätzlich sollten für Tests Heuristiken oder "Smells" beachtet werden.
T1: Unzureichende Tests
Eine Testreihe sollte alle möglichen Ausführungen von Funktionen testen. Oft sind Programmierer schon nach den Test ob es für ihren Fall funktioniert fertig. Korrekt ist es jedoch erst wenn der gesammte bereich an Möglichkeiten abgedeckt ist

T2: Nutzung von Abdeckungs Tools
Oftmals helfen Entwicklungsumgebungen bereits nicht getestete If cases und Catch Statements zu erkennen. Diese Praxis sichert den Vollständigen Test aller Funktionen sicher.

T3: Kein Überspringen von Triviale Tests 
...
T5: Rahmenbedingungen Testen
Oftmals befinden sich potentielle Fehler an den Rändern der Eingabebereiche durch ein Testen der Rahmenbedingungen fallen diese schnell auf.
...
Wenn ein Bug auffällt ist es meist einfacher erst in Tests die Bedinungen für einen Fehler in Tests zu dokumentieren und danach mit dem gewonnen Wissen den Fehler zu beheben.

T8: Testabdeckung zeigt Logikfehler
Durch eine Hohe Testabdeckung werden fehlerhafte Statements besser sichtbar. So wird durch viele Tests sichtbar welche Funktionen oder Abläufe zu Fehlern führen.
Oft wird durch viele Tests ein Muster sichtbar nach welchem Funktionen oder Fallunterscheidungen nicht nach erwarten funktionieren.

T9: Test müssen schnell sein
Langsame Tests werden oft wegoptimiert. Wenn ein Test nicht schnellgenug ausführbar ist ist die warscheinlichkeit groß das dieser in den folgenden Ausführungen weggelassen wird.


=== Test Coverage
Um vernünfig Testen zu können muss als erstes evaluiert werden welche Teile des Codes wie getestet werden.
Da für die Klasse bereits JUnit tests bestehen werden diese als Grundlage genutzt.
...
Auch alternative Eingaben können um Mehrdeutigkeiten zu erfassen in den Programmablauf mit eingearbeitet werden.
TODO: bsp text eingabe Ignore Case
...
Dieser stelle muss mit besonderer Vorsicht herangegangen werden da der Algoritmus nicht zu 100% funktionsfähig ist und Änderungen schnell zu neuen Fehlern führen.
